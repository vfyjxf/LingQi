...
export default function ReviewSummary({ summary, generalSuggestions, meta }: ReviewSummaryProps) {
  return (
    <div className="space-y-6">
      {/* Meta header */}
      <div className="flex flex-col gap-4 rounded-md border border-[#30363d] bg-[#161b22] p-6 shadow-sm md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          {meta.avatarUrl ? (
            <img src={meta.avatarUrl} alt={meta.author} className="h-12 w-12 rounded-full border border-[#30363d]" referrerPolicy="no-referrer" />
          ) : (
            <div className="flex h-12 w-12 items-center justify-center rounded-full border border-[#30363d] bg-[#21262d]">
              <User className="h-6 w-6 text-[#8b949e]" aria-hidden="true" />
            </div>
          )}
          <div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <div className="space-y-4 rounded-md border border-[#30363d] bg-[#161b22] p-6 shadow-sm md:col-span-2">
          ...
          <p className="text-sm leading-relaxed whitespace-pre-wrap text-[#c9d1d9]">
            {summary.overview || "无 PR 变更描述。"}
          </p>
        </div>

        <div className="space-y-4 rounded-md border border-[#30363d] bg-[#161b22] p-6 shadow-sm">
          <h3 className="flex items-center gap-2 border-b border-[#30363d] pb-3 text-sm font-semibold text-[#c9d1d9]">
          ...
          <div className="space-y-2">
            {summary.changedModules.length > 0 ? (
              ...
              <p className="text-xs italic text-[#8b949e]">无模块信息。</p>
            )}
          </div>
        </div>
      </div>
      ...
      {summary.testSummary && (
        <div className="rounded-md border border-[#30363d] bg-[#161b22] p-6 shadow-sm space-y-4">
          <h3 className="flex items-center gap-2 border-b border-[#30363d] pb-3 text-sm font-semibold text-[#c9d1d9]">
            <FileText className="h-4 w-4 text-[#8957e5]" />
            测试摘要
          </h3>
          <p className="text-sm leading-relaxed text-[#c9d1d9]">{summary.testSummary}</p>
        </div>
      )}
