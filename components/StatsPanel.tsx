...
/* ---- Quality score ---- */
export function calcQualityScore(stats: StatsData) {
  const deductions =
...
export function gradeInfo(score: number) {
...
export function GradeBadge({ stats }: { stats: StatsData }) {
  const score = calcQualityScore(stats);
  const grade = gradeInfo(score);

  return (
    <div className="relative flex flex-col items-center justify-between overflow-hidden rounded-lg border border-[#30363d] bg-gradient-to-br from-[#161b22] to-[#0d2840] p-6 text-center shadow-sm">
...
export function StatsCharts({ stats, activeFilter, onFilterChange }: { stats: StatsData; activeFilter?: FilterState; onFilterChange?: (type: "category" | "severity" | "clear", value: string | null) => void }) {
  ...
  return (
    <div className="flex gap-4">
      {/* Severity donut */}
      <div className="flex-1 flex flex-col justify-between rounded-lg border border-[#30363d] bg-[#161b22] p-6 shadow-sm">
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value, _name, props) => {
                    ...
                    const label = (props as TooltipPayloadProps).payload?.name ?? "";
