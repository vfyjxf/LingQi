...
export function buildReviewPrompt(context: PrAnalysisContext): string {
  ...
  return [
    ...
    "- 优先阅读每个文件的 numberedPatch：`+ RIGHT N |` 表示新增侧第 N 行，`  LEFT M RIGHT N |` 表示上下文行，`- LEFT M |` 表示删除侧行。",
    "- risks.line 和 suggestions.line 必须取自问题证据所在代码行旁边的 RIGHT 行号，并且必须是对应文件 commentableLines 数组中的数字。",
    "- 不要使用 oldLines、LEFT 行号、diff 显示序号、删除行号、hunk 头行号或推测行号。",
    "- 如果问题只定位到文件、删除行或上下文不足，省略 line 字段，不要为了生成 inline comment 编造行号。",
